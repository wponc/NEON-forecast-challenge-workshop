...
```{r}
met_variables <- c("relative_humidity")
```
...
for(i in 1:length(lake_sites$field_site_id)) {  
  ...
  #### CHANGED LEFT SIDE TO OXYGEN AND RIGHT SIDE TO RELATIVE HUMIDIT###
  ...
  residuals <- mod - site_target$oxygen
  ...
  forecasted_oxygen <- param_df$beta1 + param_df$beta2 * site_drivers$relative_humidity + rnorm(n = n_members, mean = 0, sd = sigma) 
...
```{r}
my_model_id <- 'example_WP_SP24_Uncertainty'
```
